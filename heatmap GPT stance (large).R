...
# Filter data for specific affiliations, considerations, and large projects (installed_capacity >= 20 MW)
...
print("Data filtered for large projects")
...
# Calculate the average GPT stance for each combination of affiliation and consideration for large projects
average_stance_large_projects <- filtered_melted_data %>%
  group_by(affiliation, variable) %>%
  summarise(avg_stance = mean(GPT_stance_numeric, na.rm = TRUE), .groups = 'drop')
print("Average GPT stance calculated for large projects")

# Create the heatmap for large projects
p_large_projects <- ggplot(average_stance_large_projects, aes(x = variable, y = affiliation, fill = avg_stance)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "red", high = "green", limits = c(-1, 1), 
                      breaks = c(-1, 0, 1), 
                      labels = c("Strong Oppose", "Neutral", "Strong Support"), 
                      name = "GPT Stance") +
  labs(x = "Consideration", y = "Affiliation", title = "Large Projects (Installed Capacity >= 20 MW) - GPT Stance Detection") +
  theme_minimal() +
  theme(
