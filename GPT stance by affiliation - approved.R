...
# Filter data for projects with project_status == "approved"
approved_projects <- subset(projects, project_status == "approved")
filtered_quotes <- subset(all_quotes, project_code %in% approved_projects$plan_number)
print("Data filtered for approved projects")

# Verify the filtered data
print("Filtered data verification:")
print(table(filtered_quotes$affiliation, filtered_quotes$GPT_stance_detection))

# Calculate the count of each GPT stance by affiliation for approved projects
...
# Create the stacked percentage bar chart for approved projects
p_stacked <- ggplot(stance_percentages, aes(x = affiliation, y = percentage, fill = GPT_stance_detection)) +
  ...
  labs(x = "Affiliation", y = "Percentage", fill = "Stance", title = "Share of Statements by GPT Stance Detection for approved Projects") +
  theme_minimal() +
  theme(
