...
# Filter data for the specific affiliations, considerations, and small projects (installed capacity < 20)
...
# Filter data for at least 3 observations per affiliation-consideration combination
min_observations <- 3
...
filtered_melted_data$variable <- gsub("_", " ", filtered_melted_data$variable)
filtered_melted_data$variable <- tolower(filtered_melted_data$variable)
filtered_melted_data$variable <- tools::toTitleCase(filtered_melted_data$variable)

# Calculate the average sentiment for each combination of affiliation and consideration for small projects
average_sentiment_small <- filtered_melted_data %>%
  group_by(affiliation, variable) %>%
  summarise(avg_sentiment = mean(sentiment), .groups = 'drop')
print("Average sentiment calculated for small projects")

# Cap the avg_sentiment at 0
average_sentiment_small$avg_sentiment <- pmin(average_sentiment_small$avg_sentiment, 0)

# Create the heatmap for small projects
p_small <- ggplot(average_sentiment_small, aes(x = variable, y = affiliation, fill = avg_sentiment)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "red", high = "green", limits = c(-1, 0), 
                      breaks = c(-1, -0.5, 0), 
                      labels = c("Negative", "Neutral", "Positive"), 
                      name = "Sentiment") +
  labs(x = "Consideration", y = "Affiliation", title = "Small projects") +
  theme_minimal() +
  theme(
