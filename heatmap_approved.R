...
# Filter data for the specific affiliations, considerations, and approved projects (project_status = 'approved')
...
filtered_quotes <- subset(all_quotes, affiliation %in% desired_affiliations & project_code %in% approved_projects$plan_number)
print("Data filtered")
...
# Calculate the average sentiment for each combination of affiliation and consideration for approved projects
average_sentiment_approved <- filtered_melted_data %>%
  group_by(affiliation, variable) %>%
  summarise(avg_sentiment = mean(sentiment), .groups = 'drop')
print("Average sentiment calculated for approved projects")

# Cap the avg_sentiment at 0
average_sentiment_approved$avg_sentiment <- pmin(average_sentiment_approved$avg_sentiment, 0)

# Create the heatmap for approved projects
p_approved <- ggplot(average_sentiment_approved, aes(x = variable, y = affiliation, fill = avg_sentiment)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "red", high = "green", limits = c(-1, 0), 
                      breaks = c(-1, -0.5, 0), 
                      labels = c("Negative", "Neutral", "Positive"), 
                      name = "Sentiment") +
  labs(x = "Consideration", y = "Affiliation", title = "Approved projects") +
  theme_minimal() +
  theme(
