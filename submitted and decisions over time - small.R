...
# Create the chart
small_projects_chart <- ggplot(long_data, aes(x = year, y = count, fill = stage, pattern = stage)) +
  geom_bar_pattern(
    stat = "identity", 
  ...
  scale_y_continuous(breaks = seq(0, max(long_data$count, na.rm = TRUE) + 5, by = 5), 
                     minor_breaks = NULL) +
