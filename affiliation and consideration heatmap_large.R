...
# Filter data for the specific affiliations, considerations, and large projects (installed capacity >= 20)
...
filtered_quotes <- subset(all_quotes, affiliation %in% desired_affiliations & project_code %in% large_projects$plan_number)
print("Data filtered")
...
# Calculate the average sentiment for each combination of affiliation and consideration for large projects
average_sentiment_large <- filtered_melted_data %>%
  group_by(affiliation, variable) %>%
  summarise(avg_sentiment = mean(sentiment), .groups = 'drop')
print("Average sentiment calculated for large projects")

# Cap the avg_sentiment at 0
average_sentiment_large$avg_sentiment <- pmin(average_sentiment_large$avg_sentiment, 0)

# Create the heatmap for large projects
p_large <- ggplot(average_sentiment_large, aes(x = variable, y = affiliation, fill = avg_sentiment)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "red", high = "green", limits = c(-1, 0), 
                      breaks = c(-1, -0.5, 0), 
                      labels = c("Negative", "Neutral", "Positive"), 
                      name = "Sentiment") +
  labs(x = "Consideration", y = "Affiliation", title = "Large projects") +
  theme_minimal() +
  theme(
